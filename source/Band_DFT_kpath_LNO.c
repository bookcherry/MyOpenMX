/**********************************************************************
  Band_DFT_kpath_LNO.c:

     Band_DFT_kpath_LNO.c is a subroutine to calculate eigenenergies
     at given k-points for writing the band dispersion 
     with localized natural orbitals (LNOs).

     input: information of k points
     output: None
             file.Band

  Log of Band_DFT_kpath_LNO.c:

     05/May/2018  Released by by T.Ozaki
                         ...
                         double ****CntOLP)
{
  ...
  if (SpinP_switch==0 || SpinP_switch==1){
    ...
    printf("The calculation of band dispersion with LNOs is not supported.\n");
    MPI_Finalize();
    exit(0);
                         ...
                         double ****CntOLP)
{
  ...
  ****************************************************/
  ...
  if (firsttime) {
  PrintMemory("Band_DFT_kpath_LNO: ko",sizeof(double)*List_YOUSO[23]*(n+1),NULL);
  }
  ...
  if (firsttime)
  PrintMemory("Band_DFT_kpath_LNO: D",
  ...
  *****************************************************/
  ...
  for (kloop0=0; kloop0<num_kloop0; kloop0++){
	...
	if (0<=kloop){
	  ...
	  Matrix_Band_LNO(ID,spin,CntOLP,TmpM,MP,k1,k2,k3);
	  n = TmpM[0][0].r;
	...
	for (i1=1; i1<=n; i1++){
	  ...
	  k3 = kpath[ik][1][id]+
	    (kpath[ik][2][id]-kpath[ik][1][id])*(i_perk-1)/(n_perk[ik]-1);
          ...
          Matrix_Band_LNO(ID, spin, nh[spin], TmpM, MP, k1, k2, k3);
